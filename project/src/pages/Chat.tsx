...
import { getChatResponse, saveChatHistory, getChatHistory, updateChatHistory } from '../lib/api';
...
export function Chat() {
    ...
    try {
      ...
      if (user) {
        if (currentChatId) {
          ...
          // Create new chat
          await saveChatHistory(user.id, updatedMessages);
        }
  ...
  return (
    <div className={`min-h-screen mesh-background dark:bg-gray-900 p-0 md:p-4 flex flex-col transition-colors duration-200`}>
      ...
      <div className={`max-w-4xl mx-auto w-full glass-effect dark:bg-gray-800/50 rounded-none md:rounded-2xl shadow-lg overflow-hidden flex-1 border border-gray-100 dark:border-gray-700 transition-all duration-300 ${
